...
namespace SquareLagrange
{
  ...
  std::string Prob_str = "SqcPo"; //Set from CL, a unique identifier.
...
PartialAnnulusProblem<ELEMENT>::PartialAnnulusProblem()
{
 ...
 create_impenetrable_lagrange_elements(tf_b1,
                                       Bulk_mesh_pt,Surface_mesh_T_pt);
 create_impenetrable_lagrange_elements(tf_b2,
                                       Bulk_mesh_pt,Surface_mesh_T_pt);
   ...
   for (unsigned j=0;j<nnod;j++)
    {
     ...
     // Is the node also on boundary 0 or 2?
     //if ((nod_pt->is_on_boundary(3))||(nod_pt->is_on_boundary(2)))
 ...
 if(CommandLineArgs::command_line_flag_has_been_set("--f_solver"))
 {
  ...
  switch(SL::F_solver)
  {
    ...
    case 16:
      ...
      break;
    case 17:
      SL::F_str = "Frayold"; // I have no short hand for this...
      break;
    case 81:
